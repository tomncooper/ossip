...
class TestIsAuthorLine:
    ...
    def test_singular_label(self):
        assert is_author_line("Author: Jane Doe")

    def test_author_word_boundary(self):
        """Prose starting with the letters 'author' must not match."""
        assert not is_author_line("Authorization note: do X")
        assert not is_author_line("Authoritative sources: the spec")


class TestIsCoAuthorLine:
    """Tests for is_co_author_line()."""

    def test_space_form(self):
        assert is_co_author_line("Co Author: Satish Duggana")

    def test_dash_form(self):
        assert is_co_author_line("Co-Author: Satish Duggana")

    def test_plural_form(self):
        assert is_co_author_line("Co-Authors: Satish Duggana")

    def test_lowercase(self):
        assert is_co_author_line("co author: Satish Duggana")

    def test_plain_author_line_is_not_co_author(self):
        assert not is_co_author_line("Authors: Jane Doe, John Smith")
        assert not is_co_author_line("Author: Jane Doe")
...
class TestParseAuthorsFromText:
    ...
    def test_shared_paragraph_stops_at_email(self):
        """An 'Email:' field sharing the author line must not leak into names."""
        text = "Authors: John DoeEmail: john@example.com"
        assert parse_authors_from_text(text) == ["John Doe"]

    def test_singular_label(self):
        assert parse_authors_from_text("Author: Jane Doe") == ["Jane Doe"]
